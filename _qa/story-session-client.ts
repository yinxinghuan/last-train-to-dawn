...
const directory = await mkdtemp(join(tmpdir(), 'last-train-to-dawn-story-session-'))
...
try {
  ...
  const initial = actionableSave()
  const original = JSON.stringify(initial)
  const store = new Store(); const scope = 'last-train-to-dawn:deployment:owner-a:zh'
  ...
  assert.equal(head.snapshot.scene, initial.scene)
  ...
  assert.equal(head.version, initial.scene + 1)
  ...
  const foreign = await client('token-b').enroll(actionableSave(), 'owner-b-enrollment')
  ...
  const rollbackSession = await client().enroll(actionableSave(), 'rollback-enrollment')
