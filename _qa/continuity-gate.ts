...
import { lastTrainToDawn as cartridge } from '../src/story/cartridges/lastTrainToDawn'
...
const partiallyGrounded = parseStoryProtocol(`北岬死站的调度图仍铺在车厢桌面。
[choices: "检查北岬死站的调度图"|"给火星总督送量子电报"|"追赶从未出现的银河快递员"]`, cartridge.locale)
const partialNext = applyParsedScene(initial, partiallyGrounded, cartridge, '查看调度图')
assert.deepEqual(partialNext.choices.map((choice) => choice.label), ['检查北岬死站的调度图'], 'one grounded choice survives without padding or unrelated recovery')
