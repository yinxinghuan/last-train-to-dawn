...
import { createInitialSave, enterStory } from '../src/story/engine/reducer'
...
const directory = await mkdtemp(join(tmpdir(), 'last-train-to-dawn-session-process-'))
const databasePath = join(directory, 'session.sqlite')
const processes = new Set<ChildProcess>()
const actionableSave = () => enterStory(createInitialSave(lastTrainToDawn), lastTrainToDawn)
...
try {
  const first = await start(); const second = await start(); const initial = actionableSave(); const opening = initial.choices[0]!.label
  ...
  const winner = await first.client().read(enrolled.session_id); assert.equal(winner.version, initial.scene + 1); assert.equal(winner.cursor, 1)
  ...
  const recovered = await first.client().submit(rollback); assert.equal(recovered.version, initial.scene + 1); assert.equal(counts().commits, 3)
  ...
  assert.equal(durable.version, initial.scene + 1); assert.equal(durable.events[0]?.action_id, lost.action_id); assert.equal(counts().commits, 4)
  ...
  const contractOptions = { cartridge: lastTrainToDawn, databasePath, actorTokens: {}, generator: { async send(): Promise<never> { throw new Error('unused') } } }
  assert.throws(() => createStorySessionLab({ ...contractOptions, rulesetVersion: 2 }), /DATABASE_CONTRACT_MISMATCH/)
  assert.throws(() => createStorySessionLab({ ...contractOptions, cartridge: { ...lastTrainToDawn, id: 'different-game' } }), /DATABASE_CONTRACT_MISMATCH/)
