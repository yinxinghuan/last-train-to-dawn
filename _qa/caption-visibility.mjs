...
await page.locator('.st-entry .st-primary').dispatchEvent('pointerdown')
...
await page.locator('.st-composer input').waitFor({ timeout: 10_000 })
...
if (await page.getByText(/image_subject|image_prompt|请做出选择/).count()) throw new Error(`internal or redundant copy remained visible: ${JSON.stringify(await page.getByText(/image_subject|image_prompt|请做出选择/).allTextContents())}`)
