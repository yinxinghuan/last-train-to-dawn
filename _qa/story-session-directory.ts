...
try {
  ...
  const privateSave = actionableSave(); privateSave.objective = 'PRIVATE_OWNER_A_SENTINEL'
  const first = await a.enroll(privateSave, 'owner-a-first')
  const second = await a.enroll(actionableSave(), 'owner-a-second')
  const foreign = await b.enroll(actionableSave(), 'owner-b-only')
  ...
  const opened = await journal.open(actionableSave())
  ...
  const restarted = await journal.restart(actionableSave())
  ...
  enrollmentStore.raw = JSON.stringify({ schema: 1, scope, enrollment: { id: 'new-session', initialSave: actionableSave() } })
