...
import { createStoryActionId, StorySessionClient, StorySessionRequestError, type StorySessionDirectory, type StorySessionHead, type StorySessionPendingAction, type StorySessionPendingEnding } from './storySessionClient'

export interface StorySessionJournalStore { read(): string | null; write(value: string): void }
interface Checkpoint { schema: 1; scope: string; enrollment?: { id: string; initialSave: StorySave }; head?: StorySessionHead; pending?: StorySessionPendingAction; pendingEnding?: StorySessionPendingEnding }
...
export class StorySessionJournal {
  ...
  private load(): Checkpoint | undefined {
    ...
    if (checkpoint.schema !== 1 || checkpoint.scope !== this.scope || (!checkpoint.head && !checkpoint.enrollment)
      || (checkpoint.pending && checkpoint.pending.session_id !== checkpoint.head?.session_id)
      || (checkpoint.pendingEnding && checkpoint.pendingEnding.session_id !== checkpoint.head?.session_id)
      || (checkpoint.pending && checkpoint.pendingEnding)) throw new Error('JOURNAL_CHECKPOINT_MISMATCH')
    return checkpoint
  }
  ...
  private async recoverEnding(checkpoint: Checkpoint) {
    const pending = checkpoint.pendingEnding!
    const current = await this.client.read(pending.session_id)
    if (current.snapshot.finale.status === 'complete' && current.snapshot.finale.snapshot?.id === pending.snapshot_id) {
      this.save({ schema: 1, scope: this.scope, head: current })
      return current
    }
    await this.client.submitEnding(pending)
    const head = await this.client.read(pending.session_id)
    this.save({ schema: 1, scope: this.scope, head })
    return head
  }
  ...
  private async resume(initialSave?: StorySave): Promise<StorySessionHead> {
    ...
    if (checkpoint.pendingEnding) return this.recoverEnding(checkpoint)
  ...
  restart(initialSave: StorySave): Promise<StorySessionHead> {
    return this.exclusive(async () => {
      ...
      if (checkpoint?.pendingEnding) throw new Error('PENDING_ENDING_REQUIRES_RECOVERY')
  ...
  finishEnding(snapshotId: string, displayed?: Pick<StorySessionHead, 'session_id' | 'version'>): Promise<StorySessionHead> {
    return this.exclusive(async () => {
      const checkpoint = this.load()
      if (!checkpoint?.head) throw new Error('SESSION_NOT_OPEN')
      if (checkpoint.pending) throw new Error('PENDING_ACTION_REQUIRES_RECOVERY')
      ...
      const next = { ...checkpoint, pendingEnding: this.client.prepareEnding(checkpoint.head, snapshotId) }
      this.save(next)
      return this.recoverEnding(next)
    })
  }
}
