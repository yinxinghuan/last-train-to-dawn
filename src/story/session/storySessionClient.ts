...
export interface StorySessionPendingEnding {
  session_id: string
  ending_id: string
  expected_version: number
  ruleset_version: number
...
function checkedHead(value: unknown, sessionId?: string): StorySessionHead {
  ...
  if (!head || typeof head.session_id !== 'string' || !head.session_id
    ...
    || head.snapshot?.version !== 8 || !Array.isArray(head.snapshot.blocks)
...
export class StorySessionClient {
  ...
  prepareEnding(head: StorySessionHead, snapshotId: string, endingId = createStoryActionId()): StorySessionPendingEnding {
    if (!stableEndingId(snapshotId) || !stableEndingId(endingId)) throw new Error('INVALID_ENDING_REQUEST')
    return { session_id: head.session_id, ending_id: endingId, expected_version: head.version,
      ruleset_version: head.ruleset_version || this.rulesetVersion, snapshot_id: snapshotId }
  }

  async submitEnding(pending: StorySessionPendingEnding): Promise<StorySessionHead> {
    const path = `/api/story/sessions/${encodeURIComponent(pending.session_id)}/ending`
    const body = JSON.stringify({ ending_id: pending.ending_id, expected_version: pending.expected_version,
      ruleset_version: pending.ruleset_version, snapshot_id: pending.snapshot_id })
    try { return checkedHead(await this.transport.request(path, { method: 'POST', body }), pending.session_id) }
    catch (cause) {
      if (cause instanceof StorySessionRequestError && cause.code !== 'VERSION_CONFLICT') throw cause
      const reconciled = await this.read(pending.session_id)
      if (reconciled.snapshot.finale.status === 'complete'
        && reconciled.snapshot.finale.snapshot?.id === pending.snapshot_id) return reconciled
      if (cause instanceof StorySessionRequestError) throw cause
      return checkedHead(await this.transport.request(path, { method: 'POST', body }), pending.session_id)
    }
  }
