...
export function StoryGameView({ cartridge, engine, player, onSelect, onLocaleChange, uiVariant }: { cartridge: StoryCartridge; engine: StoryEngineView; player: PlayerProfile; onSelect: (id: string) => void; onLocaleChange: (locale: Locale) => void; uiVariant: UiVariant }) {
  ...
  const stage = <CinematicStage cartridge={cartridge} engine={engine} player={player} previewScene={previewScene} onReturnLatest={() => setPreviewScene(null)} onOpeningReadyChange={setOpeningReady} onResultReadyChange={setResultNarrativeReady} uiVariant={uiVariant} turnPhase={turnPhase} lastAction={lastAction} />
