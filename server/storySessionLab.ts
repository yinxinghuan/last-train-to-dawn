...
import type { StoryCartridge, StoryEnding, StoryEndingSnapshot, StorySave } from '../src/story/types'
...
import { buildEndingSnapshot, validateEndingCandidate } from '../src/story/engine/endingDirector'
...
interface Head { sessionId: string; owner: string; rulesetVersion: number; version: number; cursor: number; snapshot: StorySave; events: Event[]; cache: Record<string, { fingerprint: string; response: StorySessionHead }>; endingCache: Record<string, { fingerprint: string; response: StorySessionHead }> }
...
export const LAST_TRAIN_TO_DAWN_SAVE_REPAIR_MIGRATION_ID = 'last-train-to-dawn-save-v8-repair-2026-09-04'
...
export function createStorySessionLab(options: {
  ...
  endingGenerator?: { generate(save: StorySave, cartridge: StoryCartridge): Promise<{ ending: StoryEnding; snapshot: StoryEndingSnapshot; usedFallback: boolean; errors: string[] }> }
  beforeEndingCommit?: (endingId: string) => Promise<void>; failEndingBeforeCommit?: string[]; dropEndingResponseAfterCommit?: string[]
}) {
  ...
  const failEndingBeforeCommit = new Set(options.failEndingBeforeCommit ?? []); const dropEndingAfterCommit = new Set(options.dropEndingResponseAfterCommit ?? []); const droppedEndings = new Set<string>()
  db.exec(`PRAGMA busy_timeout=5000; PRAGMA journal_mode=WAL; PRAGMA synchronous=FULL;
    ...
    CREATE TABLE IF NOT EXISTS ending_audit(session_id TEXT NOT NULL,ending_id TEXT NOT NULL,snapshot_id TEXT NOT NULL,PRIMARY KEY(session_id,ending_id));
  ...
  const insertEndingAudit = db.prepare('INSERT INTO ending_audit(session_id,ending_id,snapshot_id) VALUES(?,?,?)')
  ...
  const ownedHead = (sessionId: string, owner: string): Head | undefined => { const row = readSession.get(sessionId) as { owner: string; head_json: string } | undefined; if (row?.owner !== owner) return undefined; const head = JSON.parse(row.head_json) as Head; head.cache ??= {}; head.endingCache ??= {}; return head }
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/api/story/sessions') {
        ...
        if (!imported || imported.version !== 8 || imported.cartridgeId !== options.cartridge.id || imported.locale !== options.cartridge.locale || !counter(imported.scene) || !Array.isArray(imported.blocks) || !Array.isArray(imported.choices)) throw new RequestError(400, 'INVALID_SAVE')
        ...
        try {
          ...
          const head: Head = { sessionId: randomUUID(), owner, rulesetVersion, version: imported.scene, cursor: 0, snapshot: structuredClone(imported), events: [], cache: {}, endingCache: {} }
      ...
      const endingMatch = url.pathname.match(/^\/api\/story\/sessions\/([^/]+)\/ending$/)
      if (endingMatch) {
        if (request.method !== 'POST') { send(response, 405, { code: 'METHOD_NOT_ALLOWED' }); return }
        const sessionId = decodeURIComponent(endingMatch[1]!); const current = ownedHead(sessionId, owner)
        if (!current) { send(response, 404, { code: 'SESSION_NOT_FOUND' }); return }
        const body = await readJson(request)
        if (Object.keys(body).some(key => !['ending_id','expected_version','ruleset_version','snapshot_id'].includes(key))
          || !stableId(body.ending_id) || !stableId(body.snapshot_id) || !counter(body.expected_version) || !counter(body.ruleset_version)) throw new RequestError(400, 'INVALID_ENDING')
        const requestFingerprint = fingerprint({ expected_version: body.expected_version, ruleset_version: body.ruleset_version, snapshot_id: body.snapshot_id })
        const cached = current.endingCache[body.ending_id]
        if (cached) { send(response, cached.fingerprint === requestFingerprint ? 200 : 409, cached.fingerprint === requestFingerprint ? { ...cached.response, replayed: true } : { code: 'ENDING_ID_CONFLICT' }); return }
        ...
        if (!['ready', 'failed'].includes(current.snapshot.finale.status)) { send(response, 409, { code: 'ENDING_NOT_READY' }); return }
        const frozen = buildEndingSnapshot(current.snapshot, options.cartridge)
        if (body.snapshot_id !== frozen.id) { send(response, 409, { code: 'ENDING_SNAPSHOT_MISMATCH' }); return }
        if (!options.endingGenerator) { send(response, 503, { code: 'ENDING_UNAVAILABLE' }); return }
        let generated
        try { generated = await options.endingGenerator.generate(structuredClone(current.snapshot), options.cartridge) }
        catch { send(response, 503, { code: 'ENDING_UNAVAILABLE' }); return }
        if (fingerprint(generated.snapshot) !== fingerprint(frozen) || generated.ending.id !== frozen.id || generated.ending.snapshotId !== frozen.id
          || validateEndingCandidate(generated.ending, frozen, options.cartridge).length) { send(response, 409, { code: 'ENDING_RESULT_MISMATCH' }); return }
        await options.beforeEndingCommit?.(body.ending_id)
        db.exec('BEGIN IMMEDIATE')
        try {
          const locked = ownedHead(sessionId, owner)!; const raced = locked.endingCache[body.ending_id]
          if (raced) { db.exec('COMMIT'); send(response, raced.fingerprint === requestFingerprint ? 200 : 409, raced.fingerprint === requestFingerprint ? { ...raced.response, replayed: true } : { code: 'ENDING_ID_CONFLICT' }); return }
          if (locked.version !== current.version || buildEndingSnapshot(locked.snapshot, options.cartridge).id !== frozen.id) { db.exec('ROLLBACK'); send(response, 409, { code: 'VERSION_CONFLICT' }); return }
          locked.version += 1
          ...
          const committed = view(locked); locked.endingCache[body.ending_id] = { fingerprint: requestFingerprint, response: committed }
          writeSession.run(JSON.stringify(locked), sessionId); const committedAt = sessionNow(); if (!counter(committedAt)) throw new RequestError(500, 'INVALID_SESSION_CLOCK'); touchDirectory.run(committedAt, sessionId, owner); insertEndingAudit.run(sessionId, body.ending_id, frozen.id)
          if (failEndingBeforeCommit.delete(body.ending_id)) throw new Error('INJECTED_ENDING_FAILURE_BEFORE_COMMIT')
          db.exec('COMMIT')
          if (dropEndingAfterCommit.has(body.ending_id) && !droppedEndings.has(body.ending_id)) { droppedEndings.add(body.ending_id); response.destroy(new Error('INJECTED_ENDING_RESPONSE_LOSS_AFTER_COMMIT')); return }
      ...
      if (migrationMatch) {
        ...
        if (!current) { send(response, 404, { code: 'SESSION_NOT_FOUND' }); return }
        const body = await readJson(request)
        if (Object.keys(body).some(key => !['migration_id','expected_version','ruleset_version'].includes(key)) || body.migration_id !== LAST_TRAIN_TO_DAWN_SAVE_REPAIR_MIGRATION_ID || !counter(body.expected_version) || !counter(body.ruleset_version)) throw new RequestError(400, 'INVALID_MIGRATION')
        ...
        if (migrated.version !== 8 || migrated.cartridgeId !== options.cartridge.id || migrated.locale !== options.cartridge.locale || !Array.isArray(migrated.blocks) || !Array.isArray(migrated.choices) || !Array.isArray(migrated.inventory) || !Array.isArray(migrated.map) || !migrated.facts || !migrated.danger) throw new RequestError(409, 'MIGRATION_RESULT_INVALID')
  ...
  return {
    ...
    endingCommittedCount() { return Number((db.prepare('SELECT COUNT(*) AS n FROM ending_audit').get() as { n: number }).n) },
  }
}
